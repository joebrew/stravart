# This is helpful:https://gist.github.com/hadley/144c406871768d0cbe66b0b810160528
library(shiny)
library(httpuv)
source('global.R')
...
server <- function(input, output, session) {
  ...
  weirdy <- reactiveVal(value = FALSE)
  ...
  observe({
    ...
    message('About to get authorisation code')
    authorisation_code <- parse_authorisation_code(session)
    message('AUTHORISATION CODE IS: ', authorisation_code)
    
    # check if authorisation code is empty
    if (is.null(authorisation_code)) {
      message('---authorisation code is null')
      app_parameters$logged_in <- F
      
      message('About to run do_url_auth')
      weirdy(TRUE)
      ...
      withProgress({
        ...
        app_parameters$token_data <- post_authorisation_code(
          ...
          strava_app_client_id = 19335, #Sys.getenv('strava_app_client_id'),
          strava_app_secret = creds$app_secret #Sys.getenv('strava_app_secret')
        )
        ...
        message('About to run stravauth()')
  ...
  output$weird <- renderUI({
    ww <- weirdy()
    if(ww){
      do_url_auth(app_name = 'GPSart',
                  app_client_id = 19335, #app_parameters$token_data$strava_app_client_id,
                  app_secret = creds$app_secret, #app_parameters$token_data$strava_app_secret,
                  app_scope="activity:read_all",
                  redirect_uri = APP_URL,
